...
public class ResourcesDegreesRepository implements DegreesRepository {
    ...
    public LiveData<List<Degree>> loadDegrees() {
        ...
        new DegreesLoaderTask().execute();
        return _degrees;
    }


    private class DegreesLoaderTask extends AsyncTask<Void, Void, List<Degree>> {
                            ...
                            if (xmlElementName.equals("class")) {
                                ...
                                switch (xmlElementName) {
                                    ...
                                    case "optional":
                                        ...
                                        break;
                                    case "paths":
                                        // Passar a tag paths à frente
                                        xmlParser.nextTag();
                                        xmlElementName = xmlParser.getName();

                                        List<Integer> foundPaths = new ArrayList<>();
                                        while (!xmlElementName.equalsIgnoreCase("paths")) {
                                            foundPaths.add(Integer.valueOf(xmlParser.nextText()));
                                            xmlParser.nextTag();
                                            xmlElementName = xmlParser.getName();
                                        }

                                        if (foundPaths.isEmpty()) {
                                            degreeClass.setHasPaths(false);
                                            degreeClass.setPaths(null);
                                        } else {
                                            degreeClass.setHasPaths(true);
                                            int[] paths = new int[foundPaths.size()];
                                            for (int i = 0; i < foundPaths.size(); i++) {
                                                paths[i] = foundPaths.get(i);
                                            }
                                            degreeClass.setPaths(paths);
                                        }

                                        break;
                                    case "mandatoryPaths":
                                        // Passar a tag mandatoryPaths à frente
                                        xmlParser.nextTag();
                                        xmlElementName = xmlParser.getName();

                                        List<Integer> mandatoryFoundPaths = new ArrayList<>();
                                        while (!xmlElementName.equalsIgnoreCase("mandatoryPaths")) {
                                            mandatoryFoundPaths.add(Integer.valueOf(xmlParser.nextText()));
                                            xmlParser.nextTag();
                                            xmlElementName = xmlParser.getName();
                                        }

                                        if (mandatoryFoundPaths.isEmpty()) {
                                            degreeClass.setHasMandatoryPaths(false);
                                            degreeClass.setMandatoryPaths(null);
                                        } else {
                                            degreeClass.setHasMandatoryPaths(true);
                                            int[] paths = new int[mandatoryFoundPaths.size()];
                                            for (int i = 0; i < mandatoryFoundPaths.size(); i++) {
                                                paths[i] = mandatoryFoundPaths.get(i);
                                            }
                                            degreeClass.setMandatoryPaths(paths);
                                        }

                                        break;
                                    case "exclusivePath":
                                        // Passar a tag mandatoryPaths à frente
                                        String exclusivePath = xmlParser.nextText();
                                        if (exclusivePath.isEmpty()) {
                                            degreeClass.setHasExclusivePaths(false);
                                            degreeClass.setExclusivePath(0);
                                        } else {
                                            degreeClass.setHasExclusivePaths(true);
                                            degreeClass.setExclusivePath(Integer.valueOf(exclusivePath));
                                        }
//                                        xmlParser.nextTag();
                                        break;
